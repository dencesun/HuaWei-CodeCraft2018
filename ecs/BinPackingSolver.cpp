		...
		for (int u = 0; u <= U; ++u) {
			for (int v = 0; v <= V; ++v) {
				if (i == 0 || u == 0 || v == 0) {
					...
					//tbl[i][u][v] = max(1 + tbl[i-1][u-items[i-1].first][v-items[i-1].second], tbl[i-1][u][v]);
					//tbl[i][u][v] = max(items[i-1].first+tbl[i-1][u-items[i-1].first][v-items[i-1].second], tbl[i-1][u][v]);
					//tbl[i][u][v] = max(items[i-1].second+tbl[i-1][u-items[i-1].first][v-items[i-1].second], tbl[i-1][u][v]);
...
vector<vector<vector<int>>> solveBinPacking(int U, int V) {
	...
	while (true) {
		...
		// cout << "ecsSelected: " << endl;
		// for (int i = 0; i < ecsSelected.size(); ++i) {
		// 	cout << ecsSelected[i] << endl;
		// }
		for (int i = 0; i < 3; ++i) {
